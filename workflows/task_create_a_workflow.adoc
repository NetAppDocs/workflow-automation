...
The workflow name and description must be unique for each workflow.
 ...
 .. *Optinal:* Specify the entity version.
 .. *Optinal:* Clear the *Consider Reserved Elements* check box if you do not want to use the reservation capability.
 .. *Optinal:* Clear the *Enable element existence validation* check box if you do not want to enable validation for elements that exist with the same name.
